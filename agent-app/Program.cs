...
public class Program
{
    ...
    private const string PickNextAgentPromptTemplate =
        ...
        - After user input, it is {{{WriterName}}}'s turn.
    ...
    private static async Task<string> GetChatMessageContentsAsync(string input, Func<Kernel> newKernel)
    {
        ...
        KernelFunctionSelectionStrategy selectionStrategy = new(pickNextAgentFunction, newKernel())
        {
        ...
        KernelFunctionTerminationStrategy terminationStrategy = new(isChatDoneFunction, newKernel())
        {
            Agents = [reviewer],
